...
const Init=DiscordBase.Init=async()=>{
  ...
  Files.saveData=(file,callback=() => {}) => {
    let writer=Files.writers[file];
    if(!writer) {writer=Files.fstorm(Files.path.join(process.cwd(),'data',`${file}.json`));}
    writer.write(JSON.stringify(Files.data[file]),callback.bind(Files));
  };
  ...
  let Config=DiscordBase.Config=await(async function(){
    const {formatDirJSON,path}=Files;
    return {...this,...(await formatDirJSON(path.join(__dirname,'config')))}
  })();
  let Database=DiscordBase.Database=await(async function(){
    const mongoClient=await MongoClient.connect(Config.bot.dbURI,{ useUnifiedTopology: true });
    const db=await mongoClient.db('discord');
    console.log(`${console.chalk.blue("Connected To")} ${console.chalk.greenBright.bold.underline(Config.bot.dbURI)}`)
    return db;
  })();
  ...
  Bot.checkCommand=function(msg) {
    ...
    if(!command) {return;}
    let cmdName=command.reduce((prev,cur,i) => {
